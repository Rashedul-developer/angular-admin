...
@Component({
  selector: 'app-login-notify',
  templateUrl: './login-notify.component.html',
  styleUrls: ['./login-notify.component.scss']
})
export class LoginNotifyComponent implements OnInit {
  ...
  changedPageItem(event) {
    ...
    allData(postData) {
      this.dataService.post(postData, 'system/settings/notifylist')
      .subscribe(data => {
  ...
  allUpdateData(postData) {
    this.dataService.post(postData, 'system/settings/notifylistUpdateData')
    .subscribe(data => {
    if (data.response === 200) {
  ...
  */
  openDialog() {
    const dialogRef = this.dialog.open(LoginNotifyDialog, {
    width: '60%',
      data: {
      id: '',
      login_with: '',
  ...
  */
  EditDialog(inputData) {
    ...
    data: {
      id:                    inputData.id,
      login_with:            inputData.login_with,
  ...
  */
  @Component({
  selector: 'login-notify-dialog',
  templateUrl: './login-notify-dialog.html',
  styles: ['.mat-form-field-appearance-outline .mat-form-field-label {top: 1.84375em !important;}']
  })
  
  export class LoginNotifyDialog {
    submitButtonEnable = false;
    type = this.data.save_type;
    
    
    constructor(public dialogRef: MatDialogRef<LoginNotifyDialog>,
    ...
    this.dataService.post(this.data, 'system/settings/notifylistInsertData')
    .subscribe(data => {
    if (data.response === 200) {
