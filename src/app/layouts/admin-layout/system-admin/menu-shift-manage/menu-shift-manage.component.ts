...
export class MenuShiftManageComponent implements OnInit {
  ...
  updateShiftFrom(menuListFrom){            // update label and value for cascade 
    menuListFrom.forEach(element => {
  ...
  updateShiftTo(menuListTo){
    menuListTo.forEach(element => {
      element.label = element.component_name;
      element.value = element.id;
      if(element.children.length>0){                                                  
        element.children.forEach(child => {
...
*/
  ...
  openEditMenuShiftModal(node:any){
    console.log(node);
    console.log(JSON.parse(node.shiftToData));
...
export class MenuShiftModal {
  ...
  onShiftFromChanges(values: any): void {
    ...
    console.log(values, this.data.shiftFromId);
    console.log(this.data.shiftFromId);
  }
  onShifToChanges(values: any): void {
    console.log(values, this.data.shiftToId);
  }
