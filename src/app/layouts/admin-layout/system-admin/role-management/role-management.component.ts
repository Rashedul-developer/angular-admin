...
export class RoleCreateComponent implements OnInit {
    ...
    permissionArray(event:any, id:number){
        if(event.checked == true)
        {
            ...
            const data = this.permissionCollection.find(o => o === id);
    ...
    formatComponent(component_collection: any){
        component_collection.forEach((firstLayer, i) => {
            firstLayer.permissions.forEach(permission => {
                permission.is_permitted = 0;
            });
            if(firstLayer.is_component == 0){
                firstLayer.childs.forEach(secondLayer =>{
                    secondLayer.permissions.forEach(permission =>{
                        permission.is_permitted = 0;
                    });
                    if(secondLayer.is_component == 0){
                        secondLayer.childs.forEach(thirdLayer =>{
                            thirdLayer.permissions.forEach(permission =>{
                                permission.is_permitted = 0;
                            });
                            if(thirdLayer.is_component == 0){
                                thirdLayer.childs.forEach(fourthLayer=>{
                                    fourthLayer.permissions.forEach(permission=>{
                                        permission.is_permitted = 0;
                                    });
                                });
                            }
    ...
    checkAll(e) {
        if(e.checked){        
            ...
            this.component_collection.forEach((firstLayer, i) => {
                firstLayer.permissions.forEach(permission => {
...
export class RoleEditComponent implements OnInit {
    ...
    getDataById(){
        ...
        .subscribe(data => {
                ...
                if (data.code === 200) {
                    ...
                    this.permissionCollection = [];
                    data.data.role.permissions.forEach(element=>{
    ...
    formatComponent(component_collection: any){
        component_collection.forEach((firstLayer, i) => {
            firstLayer.permissions.forEach(permission => {
                const data = this.permissionCollection.find(o => o === permission.id);
                if(data){
                    permission.is_permitted = 1;
                }
                else{
